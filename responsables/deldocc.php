...
$query_verificaritem = sprintf("SELECT * FROM carpeta WHERE codigo_carpeta = %s ", GetSQLValueString($colname_verificaritem, "text"));
...
if($row_verificaritem['id_carpeta'] > 0 && $row_verificaritem['codigo_carpeta'] !='')
{
	
if ((isset($_GET['item'])) && ($_GET['item'] != "")) {
  
	  $deleteSQL = sprintf("DELETE FROM carpeta WHERE codigo_carpeta=%s",
                       GetSQLValueString($_GET['item'], "text"));
  

  mysqli_select_db($conexion, $database_conexion);
  $Result1 = mysqli_query($conexion, $deleteSQL) or die(mysqli_error($conexion));
    $deleteGoTo = "carpetas.php?item=".$row_verificaritem['cod_meta_carp'];
